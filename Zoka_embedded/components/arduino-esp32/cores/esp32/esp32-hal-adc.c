...
static bool adcDetachBus(void *pin) {
  ...
  for (uint8_t channel = 0; channel < SOC_ADC_CHANNEL_NUM(adc_unit);
       channel++) {
    int io_pin;
  ...
  if (used_channels == 1) { // only 1 channel is used
    esp_err_t err =
        adc_oneshot_del_unit(adc_handle[adc_unit].adc_oneshot_handle);
    if (err != ESP_OK) {
      return false;
    }
    ...
    if (adc_handle[adc_unit].adc_cali_handle != NULL) {
      ...
      err = adc_cali_delete_scheme_curve_fitting(
          adc_handle[adc_unit].adc_cali_handle);
      if (err != ESP_OK) {
        return false;
      }
...
esp_err_t __analogChannelConfig(adc_bitwidth_t width, adc_attenuation_t atten,
                                int8_t pin) {
  ...
  if (pin == -1) { // Reconfigure all used analog pins/channels
      ...
      if (adc_handle[adc_unit].adc_oneshot_handle != NULL) {
        for (uint8_t channel = 0; channel < SOC_ADC_CHANNEL_NUM(adc_unit);
          ...
          if (perimanGetPinBusType(io_pin) == ESP32_BUS_TYPE_ADC_ONESHOT) {
            err = adc_oneshot_config_channel(
                adc_handle[adc_unit].adc_oneshot_handle, channel, &config);
            if (err != ESP_OK) {
        ...
        if (adc_handle[adc_unit].adc_cali_handle != NULL) {
          ...
          err = adc_cali_delete_scheme_curve_fitting(
              adc_handle[adc_unit].adc_cali_handle);
          if (err != ESP_OK) {
            log_e("adc_cali_delete_scheme_curve_fitting failed with error: %d",
                  err);
            return err;
          }
          ...
          err = adc_cali_create_scheme_curve_fitting(
              &cali_config, &adc_handle[adc_unit].adc_cali_handle);
          if (err != ESP_OK) {
            log_e("adc_cali_create_scheme_curve_fitting failed with error: %d",
                  err);
            return err;
          }
#elif !defined(                                                                \
    CONFIG_IDF_TARGET_ESP32H2) // ADC_CALI_SCHEME_LINE_FITTING_SUPPORTED
          log_d("Deleting ADC_UNIT_%d line cali handle", adc_unit);
          err = adc_cali_delete_scheme_line_fitting(
              adc_handle[adc_unit].adc_cali_handle);
          if (err != ESP_OK) {
            log_e("adc_cali_delete_scheme_line_fitting failed with error: %d",
                  err);
            return err;
          }
          ...
          err = adc_cali_create_scheme_line_fitting(
              &cali_config, &adc_handle[adc_unit].adc_cali_handle);
          if (err != ESP_OK) {
            log_e("adc_cali_create_scheme_line_fitting failed with error: %d",
                  err);
            return err;
          }
#endif
    ...
    if (perimanGetPinBusType(pin) == ESP32_BUS_TYPE_ADC_ONESHOT) {
      ...
      err = adc_oneshot_config_channel(adc_handle[adc_unit].adc_oneshot_handle,
                                       channel, &config);
      if (err != ESP_OK) {
        log_e("adc_oneshot_config_channel failed with error: %d", err);
        return err;
...
esp_err_t __analogInit(uint8_t pin, adc_channel_t channel,
                       adc_unit_t adc_unit) {
  esp_err_t err = ESP_OK;
  if (adc_handle[adc_unit].adc_oneshot_handle == NULL) {
    ...
    err = adc_oneshot_new_unit(&init_config1,
                               &adc_handle[adc_unit].adc_oneshot_handle);
...
uint32_t __analogReadMilliVolts(uint8_t pin) {
  ...
  err = adc_oneshot_get_calibrated_result(
      adc_handle[adc_unit].adc_oneshot_handle,
      adc_handle[adc_unit].adc_cali_handle, channel, &value);
  if (err != ESP_OK) {
...
extern uint16_t analogRead(uint8_t pin)
    __attribute__((weak, alias("__analogRead")));
extern uint32_t analogReadMilliVolts(uint8_t pin)
    __attribute__((weak, alias("__analogReadMilliVolts")));
extern void analogReadResolution(uint8_t bits)
    __attribute__((weak, alias("__analogReadResolution")));
extern void analogSetAttenuation(adc_attenuation_t attenuation)
    __attribute__((weak, alias("__analogSetAttenuation")));
extern void analogSetPinAttenuation(uint8_t pin, adc_attenuation_t attenuation)
    __attribute__((weak, alias("__analogSetPinAttenuation")));

#if CONFIG_IDF_TARGET_ESP32
extern void analogSetWidth(uint8_t bits)
    __attribute__((weak, alias("__analogSetWidth")));
#endif
...
static bool adcContinuousDetachBus(void *adc_unit_number) {
  ...
  if (adc_handle[adc_unit].adc_continuous_handle == NULL) {
    ...
    esp_err_t err =
        adc_continuous_deinit(adc_handle[adc_unit].adc_continuous_handle);
    if (err != ESP_OK) {
      return false;
    }
...
bool IRAM_ATTR adcFnWrapper(adc_continuous_handle_t handle,
                            const adc_continuous_evt_data_t *edata,
                            void *args) {
  interrupt_config_t *isr = (interrupt_config_t *)args;
  // Check if edata->size matches conversion_frame_size, else just return from
  // ISR
...
esp_err_t __analogContinuousInit(adc_channel_t *channel, uint8_t channel_num,
                                 adc_unit_t adc_unit,
  ...
  esp_err_t err = adc_continuous_new_handle(
      &adc_config, &adc_handle[adc_unit].adc_continuous_handle);
  if (err != ESP_OK) {
  ...
  err = adc_continuous_config(adc_handle[adc_unit].adc_continuous_handle,
                              &dig_cfg);
...
bool analogContinuous(uint8_t pins[], size_t pins_count,
                      uint32_t conversions_per_pin, uint32_t sampling_freq_hz,
                      void (*userFunc)(void)) {
  ...
  if (adc_handle[adc_unit].adc_continuous_handle != NULL) {
    log_e("ADC%d continuous is already initialized. To reconfigure call "
          "analogContinuousDeinit() first.",
          adc_unit + 1);
    return false;
  }

  // Check sampling frequency
  if ((sampling_freq_hz < SOC_ADC_SAMPLE_FREQ_THRES_LOW) ||
      (sampling_freq_hz > SOC_ADC_SAMPLE_FREQ_THRES_HIGH)) {
    log_e("Sampling frequency is out of range. Supported sampling frequencies "
          "are %d - %d",
  ...
  // Set conversion frame and buffer size (conversion frame must be in multiples
  // of SOC_ADC_DIGI_DATA_BYTES_PER_CONV)
  adc_handle[adc_unit].conversion_frame_size =
      conversions_per_pin * pins_count * SOC_ADC_DIGI_RESULT_BYTES;

#if CONFIG_IDF_TARGET_ESP32 || CONFIG_IDF_TARGET_ESP32S2
  uint8_t calc_multiple = adc_handle[adc_unit].conversion_frame_size %
                          SOC_ADC_DIGI_DATA_BYTES_PER_CONV;
  if (calc_multiple != 0) {
    adc_handle[adc_unit].conversion_frame_size =
        (adc_handle[adc_unit].conversion_frame_size + calc_multiple);
  }
#endif

  adc_handle[adc_unit].buffer_size =
      adc_handle[adc_unit].conversion_frame_size * 2;
  ...
  err = __analogContinuousInit(channel, sizeof(channel) / sizeof(adc_channel_t),
                               adc_unit, sampling_freq_hz);
  if (err != ESP_OK) {
    log_e("Analog initialization failed!");
    return false;
  }
  ...
  err = adc_continuous_register_event_callbacks(
      adc_handle[adc_unit].adc_continuous_handle, &cbs,
      &adc_handle[adc_unit].adc_interrupt_handle);
  if (err != ESP_OK) {
  ...
  for (int k = 0; k < pins_count; k++) {
    if (!perimanSetPinBus(pins[k], ESP32_BUS_TYPE_ADC_CONT,
                          (void *)(adc_unit + 1), adc_unit, channel[k])) {
...
bool analogContinuousRead(adc_continuos_data_t **buffer, uint32_t timeout_ms) {
  if (adc_handle[ADC_UNIT_1].adc_continuous_handle != NULL) {
    ...
    esp_err_t err = adc_continuous_read(
        adc_handle[ADC_UNIT_1].adc_continuous_handle, adc_read,
        adc_handle[0].conversion_frame_size, &bytes_read, timeout_ms);
    if (err != ESP_OK) {
    ...
    for (int j = 0; j < used_adc_channels; j++) {
      if (read_count[j] != 0) {
        ...
        adc_cali_raw_to_voltage(adc_handle[ADC_UNIT_1].adc_cali_handle,
                                adc_result[j].avg_read_raw,
                                &adc_result[j].avg_read_mvolts);
      } else {
...
bool analogContinuousStart() {
  if (adc_handle[ADC_UNIT_1].adc_continuous_handle != NULL) {
    if (adc_continuous_start(adc_handle[ADC_UNIT_1].adc_continuous_handle) ==
        ESP_OK) {
      return true;
    }
  } else {
...
bool analogContinuousStop() {
  if (adc_handle[ADC_UNIT_1].adc_continuous_handle != NULL) {
    if (adc_continuous_stop(adc_handle[ADC_UNIT_1].adc_continuous_handle) ==
        ESP_OK) {
      return true;
    }
  } else {
...
bool analogContinuousDeinit() {
  if (adc_handle[ADC_UNIT_1].adc_continuous_handle != NULL) {
    esp_err_t err =
        adc_continuous_deinit(adc_handle[ADC_UNIT_1].adc_continuous_handle);
    if (err != ESP_OK) {
      return false;
    }
    free(adc_result);
...
void analogContinuousSetWidth(uint8_t bits) {
  if ((bits < SOC_ADC_DIGI_MIN_BITWIDTH) &&
      (bits > SOC_ADC_DIGI_MAX_BITWIDTH)) {
    log_e("Selected width cannot be set. Range is from %d to %d",
